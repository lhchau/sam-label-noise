...
class SAMAGREEMENT(torch.optim.Optimizer):
    def __init__(self, params, rho=0.05, condition=1, adaptive=False, **kwargs):
        assert rho >= 0.0, f"Invalid rho, should be non-negative: {rho}"

        defaults = dict(rho=rho, adaptive=adaptive, **kwargs)
        super(SAMAGREEMENT, self).__init__(params, defaults)
    ...
    def second_step(self, zero_grad=False):
        for group in self.param_groups:
            for p in group['params']:
                ...
                p.add_(param_state['e_w'])  # climb to the local maximum "w + e(w) + e(w)"
    ...
    def third_step(self, zero_grad=False):
        for group in self.param_groups:
            ...
            for p in group['params']:
                if p.grad is None: continue
                param_state = self.state[p]
                p.sub_(2 * param_state['e_w'])  # get back to "w" from "w + 2 * e(w)"
                
                mask = torch.logical_and( param_state['first_grad'].mul(param_state['second_grad']) > 0, param_state['first_grad'].mul(p.grad) > 0)
                d_p = torch.where(mask, p.grad, p.grad.mul(self.condition))
                if weight_decay != 0:
