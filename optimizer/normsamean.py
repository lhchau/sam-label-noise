    ...
    def second_step(self, zero_grad=False):
        for group in self.param_groups:
            for p in group['params']:
                ...
                param_state['d_t'] = p.grad.mul(mask).mul(self.condition) + p.grad.mul(torch.logical_not(mask))
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                d_p = param_state['d_t'].mul(self.second_grad_norm/self.d_t_grad_norm)
                if weight_decay != 0:
